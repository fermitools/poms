...
class TagsPOMS:
    ...
    def link_tags(self, ctx, campaign_id, tag_name):
        tag = ctx.db.query(Tag).filter(
            ...
            Tag.experiment == ctx.experiment).first()
    ...
    def search_campaigns(self, ctx, search_term):
        query = (ctx.db.query(Campaign, CampaignStage) #
                 .filter(CampaignStage.campaign_id == Campaign.campaign_id, Campaign.name.like(search_term),Campaign.experiment == ctx.experiment)
    ...
    def delete_campaigns_tags(
            self, ctx, campaign_id, tag_id, delete_unused_tag=False):
        response = {"msg": "OK"}
        campaign_ids = str(campaign_id).split(',')
        for cid in campaign_ids:
