...
class TablesPOMS:
        ...
        for fieldname in list(columns.keys()):
            ...
            if str(columns[fieldname].type) == "JSON":
                ...
                if kwargs.get(fieldname, None) is not None:
                    setattr(
                        found, fieldname, datetime.strptime(kwargs.get(fieldname, "")[:16], "%Y-%m-%d %H:%M").replace(tzinfo=utc)
                    )
            ...
            screendata.append(
                {
                    ...
                    "value": (
                        json.dumps(getattr(found, fn, "[]")) if str(columns[fn].type) == "JSON" else str(getattr(found, fn, ""))
                    ),
    ...
    def make_admin_map(self):  # This method was deleted from the main script.
        ...
        for k in dir(poms_model):
            logit.log("key: %s" % k)
            if (
                hasattr(poms_model.__dict__[k], "__module__")
