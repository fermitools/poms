...
def parse_condor_log(dbhandle, lines, batchhost):
    ...
    for line in lines:
        if line[:5] == "001 (":
            ...
            stimes[jobsub_job_id] = datetime.strptime(line[ppos+2:ppos+16],"%m/%d %H:%M:%S")
        if line[:5] == "005 (":
            if _debug: print "term record start: " , line
            ppos = line.find(")")
            in_termination = 1
            finish_time = datetime.strptime(line[ppos+2:ppos+16],"%m/%d %H:%M:%S")
            jobsub_job_id = fix_jobid(line[5:ppos], batchhost)
            remote_cpu = None
            disk_used = None
            memory_used = None
        ...
        if in_termination:
            ...
            if _debug: print "remote_cpu ", remote_cpu, " disk_used ", disk_used, " memory_used ", memory_used
