...
class Tag(Base):
    ...
    tag_id = Column(Integer, primary_key=True, server_default=text(
        "nextval('campaigns_campaign_id_seq'::regclass)"))
    experiment = Column(
        ForeignKey('experiments.experiment'),
        nullable=False,
        index=True)
    tag_name = Column(Text, nullable=False)
    creator = Column(
        ForeignKey('experimenters.experimenter_id'),
        nullable=False,
        index=True)
...
class Campaign(Base):
    ...
    campaign_id = Column(Integer, primary_key=True, server_default=text(
        "nextval('campaigns_campaign_id_seq'::regclass)"))
    experiment = Column(
        ForeignKey('experiments.experiment'),
        nullable=False,
...
class CampaignStage(Base):
    ...
    campaign_stage_id = Column(Integer, primary_key=True, server_default=text(
        "nextval('campaign_stages_campaign_stage_id_seq'::regclass)"))
    experiment = Column(
        ForeignKey('experiments.experiment'),
        nullable=False,
        index=True)
    ...
    login_setup_id = Column(
        ForeignKey('login_setups.login_setup_id'),
        nullable=False)
    ...
    experimenter_creator_obj = relationship(
        'Experimenter',
        primaryjoin='CampaignStage.creator == Experimenter.experimenter_id')
    experimenter_updater_obj = relationship(
        'Experimenter',
        primaryjoin='CampaignStage.updater == Experimenter.experimenter_id')
    experimenter_holder_obj = relationship(
        'Experimenter',
        primaryjoin='CampaignStage.hold_experimenter_id == Experimenter.experimenter_id')
...
class LoginSetup(Base):
    ...
    login_setup_id = Column(Integer, primary_key=True, server_default=text(
        "nextval('login_setups_login_setup_id_seq'::regclass)"))
    ...
    experimenter_creator_obj = relationship(
        'Experimenter',
        primaryjoin='LoginSetup.creator == Experimenter.experimenter_id')
    experimenter_updater_obj = relationship(
        'Experimenter',
        primaryjoin='LoginSetup.updater == Experimenter.experimenter_id')


class JobType(Base):
    __tablename__ = 'job_types'

    job_type_id = Column(Integer, primary_key=True, server_default=text(
        "nextval('job_types_job_type_id_seq'::regclass)"))
    ...
    experiment_obj = relationship('Experiment')
    experimenter_creator_obj = relationship(
        'Experimenter', primaryjoin='JobType.creator == Experimenter.experimenter_id')
    experimenter_updater_obj = relationship(
        'Experimenter', primaryjoin='JobType.updater == Experimenter.experimenter_id')


class Submission(Base):
    __tablename__ = 'submissions'

    submission_id = Column(Integer, primary_key=True, server_default=text(
        "nextval('submissions_submissions_id_seq'::regclass)"))
    ...
    login_setup_snapshot_id = Column(
        ForeignKey('login_setup_snapshots.login_setup_snapshot_id'),
        nullable=True,
        index=True)
    campaign_stage_snapshot_id = Column(
        ForeignKey('campaign_stage_snapshots.campaign_stage_snapshot_id'),
        nullable=True,
        index=True)
    job_type_snapshot_id = Column(
        ForeignKey('job_type_snapshots.job_type_snapshot_id'),
        nullable=True,
        index=True)
    recovery_position = Column(Integer)
    recovery_tasks_parent = Column(
        ForeignKey('submissions.submission_id'), index=True)
...
class SubmissionHistory(Base):
    ...
    status_id = Column(
        ForeignKey('submission_statuses.status_id'),
        nullable=False,
        index=True)
...
class CampaignStageSnapshot(Base):
    ...
    campaign_stage_snapshot_id = Column(Integer, primary_key=True, server_default=text(
        "nextval('campaign_stage_snapshots_campaign_stage_snapshot_id_seq'::regclass)"))
    campaign_stage_id = Column(
        ForeignKey('campaign_stages.campaign_stage_id'),
        nullable=False,
        index=True)
    ...
    # FIXME: Should it be campaign_stage?
...
class LoginSetupSnapshot(Base):
    ...
    login_setup_snapshot_id = Column(Integer, primary_key=True, server_default=text(
        "nextval('login_setup_snapshots_login_setup_id_seq'::regclass)"))
    login_setup_id = Column(
        ForeignKey('login_setups.login_setup_id'),
        nullable=False,
        index=True)
...
class RecoveryType(Base):
    ...
    recovery_type_id = Column(Integer, primary_key=True, server_default=text(
        "nextval('recovery_types_recovery_type_id_seq'::regclass)"))
    name = Column(Text, nullable=False)
...
class CampaignRecovery(Base):
    ...
    recovery_type_id = Column(
        ForeignKey('recovery_types.recovery_type_id'),
        primary_key=True,
        nullable=False,
        index=True)
...
class CampaignDependency(Base):
    ...
    campaign_dep_id = Column(Integer, primary_key=True, server_default=text(
        "nextval('campaign_dependency_id_seq'::regclass)"))
    ...
    provider = relationship(
        'CampaignStage',
        foreign_keys=needs_campaign_stage_id,
        backref='consumer_associations')
    consumer = relationship(
        'CampaignStage',
        foreign_keys=provides_campaign_stage_id,
        backref='provider_associations')
