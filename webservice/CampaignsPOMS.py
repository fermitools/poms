...
class CampaignsPOMS:
    ...
    def login_setup_edit(self, ctx, **kwargs):
        ...
        experimenter = ctx.db.query(Experimenter).filter(Experimenter.username == ctx.usernamename).first()
        data = {}
        template = None
        message = None
        ae_launch_id = None
        ...
        if action == "delete":
            self.poms_service.permissions.can_modify(ctx, "LoginSetup", name=ctx.usernamename, experiment=experiment)
            name = ae_launch_name
            try:
            ...
            if pcl_call == 1:
                if isinstance(ae_launch_name, str):
                    ae_launch_name = ae_launch_name.strip()
                name = ae_launch_name
                experimenter = ctx.db.query(Experimenter).filter(Experimenter.username == pc_ctx.usernamename).first()
                if experimenter:
    ...
    def job_type_edit(self, ctx, **kwargs):
        ...
        action = kwargs.pop("action", None)
        experimenter = ctx.db.query(Experimenter).filter(Experimenter.username == ctx.usernamename).scalar()
        ...
        if experiment:  # cuz the default is find
            ...
            recs_dict = {}
            for cid in cids:
                recs_dict[cid] = json.dumps(self.get_recoveries(ctx.db, cid))
    ...
    def campaign_stage_edit(self, ctx, **kwargs):
        ...
        if action == "delete":
            ...
            self.poms_service.permissions.can_modify(ctx, "CampaignStage", name=name, campaign_id=campaign_id, experiment=experiment)
            if isinstance(name, str):
                name = name.strip()
            if pcl_call == 1:
                campaign_stage_id = (
        ...
        if experiment:  # cuz the default is find
            if kwargs.get("update_view", None) is None:
                ...
                data["view_mine"] = experimenter.experimenter_id
                data["view_others"] = experimenter.experimenter_id
                data["view_analysis"] = "view_analysis" if role in ("analysis", "superctx.username") else None
                data["view_production"] = "view_production" if role in ("production", "superctx.username") else None
            else:
                data["view_active"] = kwargs.get("view_active", None)
                data["view_inactive"] = kwargs.get("view_inactive", None)
                data["view_mine"] = kwargs.get("view_mine", None)
    ...
    def campaign_deps_ini(self, ctx, name=None, stage_id=None, login_setup=None, job_type=None, full=None):
        ...
        for j_t in jts:
            ...
            res.append("recoveries = %s" % json.dumps(self.get_recoveries(ctx.db, j_t.job_type_id)))
            res.append("")
    ...
    def show_campaign_stages(
        ...
        **kwargs,
        ...
        (tmin, tmax, tmins, tmaxs, nextlink, prevlink, time_range_string, tdays) = self.poms_service.utilsPOMS.handle_dates(
            tmin, tmax, tdays, "show_campaign_stages/%s/%s?" % (experiment, ctx.role)
        )
        experimenter = ctx.db.query(Experimenter).filter(Experimenter.username == ctx.username).first()
    ...
    def schedule_launch(self, ctx, campaign_stage_id):
        ...
        """
        c_s = ctx.db.query(CampaignStage).filter(CampaignStage.campaign_stage_id == campaign_stage_id).first()
        my_crontab = CronTab(ctx.username=True)
    ...
    def split_type_javascript(self, ctx, **kwargs):
    ...
    def save_campaign(self, ctx, replace=False, pcl_call=0, **kwargs):
    ...
    def get_jobtype_id(self, ctx, name):
        ...
        return ctx.db.query(JobType.job_type_id).filter(JobType.experiment == experiment, JobType.name == name).scalar()

    def get_loginsetup_id(self, ctx.db, ctx.username, experiment, role, name):
        """
