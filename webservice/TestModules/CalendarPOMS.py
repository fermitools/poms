...
def calendar(self, dbhandle):
	rows = db.handle.query(Service).filter(Service.name != "All").filter(Service.name != "DCache").filter(Service.name != "Enstore").filter(Service.name != "SAM").filter(Service.name != "FifeBatch").filter(~Service.name.endswith("sam")).all()
	return rows
...
def update_service(self, dbhandle, log_handle, name, parent, status, host_site, total, failed, description):
        ...
        if s.status != status and status == "good":
            ...
            d = dbhandle.db.query(ServiceDowntime).filter(ServiceDowntime.service_id == s.service_id ).order_by(desc(ServiceDowntime.downtime_started)).first()
            if d:
...
def service_status(self, dbhandle, under = 'All'):
        prev = None
        prevparent = None
        p = dbhandle.db.query(Service).filter(Service.name == under).first()
        list = []
        for s in dbhandle.db.query(Service).filter(Service.parent_service_id == p.service_id).all():
...
def service_status_hier(self, dbhandle, under = 'All', depth = 0):
   	...
   	p = dbhandle.query(Service).filter(Service.name == under).first()
        if depth == 0:
            ...
            res = ''
        active = ""
        for s in dbhandle.query(Service).filter(Service.parent_service_id == p.service_id).order_by(Service.name).all():
