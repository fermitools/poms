...
def safe_get(req, url, *args, **kwargs):
    reply = None
    try:
        reply = req.get(url, timeout=5.0, *args, **kwargs)    # Timeout may need adjustment!
...
class samweb_lite:
    ...
    def fetch_info_list(self, task_list):
        ...
        with requests.Session() as sess:
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                ...
                replies = executor.map(lambda url: safe_get(sess, url), urls)
        infos = []
        for r in replies:
            if r:
                try:
    ...
    def list_files(self, experiment, dims):
        ...
        flist = []
        res = safe_get(requests, url, params={"dims": dims, "format": "json"})
        if res:
            try:
                flist = res.json()
    ...
    def count_files(self, experiment, dims):
        ...
        count = 0
        res = safe_get(requests, url, params={"dims": dims})
        if res:
            text = res.content
            try:
                count = int(text)
