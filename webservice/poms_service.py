...
class PomsService:
    ...
    def __init__(self):
        ...
        self.sam_base = self.web_config.get("SAM", "sam_base").replace("samsamdev","samdev")
    ...
    def ini_to_campaign(self, ctx, upload=None, **kwargs):
        ...
        for name in stage_names:
            sn = name.split(" ", 1)[1]
            if campaign_s["data_handling_service"] == "sam":
                stage_sam_config_loads = json.loads(cfg[name]["sam_settings"])
            else:
            ...
            if campaign_s["data_handling_service"] == "sam":
                campaign_d["stages"][stagenum]["form"]["sam_dispatcher_settings"] = stage_sam_config_loads
            else:
