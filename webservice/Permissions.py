...
class Permissions:
    ...
    def getBearerTokenOIDC(self, ctx, vaulttoken, vaultpath, vaultserver = "https://htvaultprod.fnal.gov:8200", oidcpath = "https:/cilogon.org/fermilab"):
        logit.log("Attempting OIDC authentication")
        if ctx.experiment != "samdev":
            oidcpath = oidcpath.replace("fermilab", ctx.experiment)
            if ctx.role != "analysis":
                oidcpath = "%spro" % oidcpath
        ...
        authdata = {
            'role': "default",
            'client_nonce': nonce,
            'redirect_uri': vaultserver + '/v1/' + oidcpath + '/callback'
        }
        ...
        if debug:
            ...
            logit.log(data)
            logit.log("##### End authentication data")
        try:
            resp = vault.request(path, data=data.encode())
        except Exception as e:
            logit.efatal("Initiating authentication to %s failed" % vaultserver, e)
        ...
        try:
            ...
            logit.efatal("decoding response from %s failed" % vaultserver, e)

        if 'data' not in response:
            logit.fatal("no 'data' in response from %s" % vaultserver)
        data = response['data']
        if 'auth_url' not in data:
            logit.fatal("no 'auth_url' in data from %s" % vaultserver)
        auth_url = data['auth_url']
        del data['auth_url'] 
        if auth_url == "":
            logit.fatal("'auth_url' is empty in data from %s" % vaultserver)

        logit.log()
        logit.log("Complete the authentication at:")
        logit.log("    " + auth_url)
        if 'user_code' in data:
            logit.log("When prompted, enter code " + data['user_code'])
            del data['user_code']
         
        opencmd = web_open_command
        if opencmd == "":
            logit.log("No web open command defined, please copy/paste the above to any web browser")
        else:
            ...
            if browser is None:
                ...
                if debug:
                    logit.log("Setting BROWSER=" + browser)
                os.putenv("BROWSER", browser)
            cp = None
            try:
                if not quiet:
                    logit.log("Running '%s' on the URL" % opencmd)
                cp = subprocess.run([opencmd, auth_url], stderr=subprocess.DEVNULL)
            except:
                pass
            if cp is None or cp.returncode != 0:
                logit.log("Couldn't open web browser with '%s', please open URL manually" % opencmd)

        if not quiet:
            logit.log("Waiting for response in web browser")
        pollinterval = 0
        ...
        while True:
            try:
                if secswaited > 120:
                    logit.fatal("Polling for response took longer than 2 minutes")
                if debug:
                ...
                logit.efatal("Authentication to %s failed" % vaultserver, e)
            body = resp.data.decode()
            if debug:
            ...
            if 'errors' in response:
                ...
                if errors[0] == "slow_down":
                    ...
                    logit.fatal("error in response from %s: %s" % (vaultserver, errors[0]))
                if debug:
...
class vaulthost:
    ...
    def __init__(self, parsedurl, capath = None, cafile = None, verbose = False,  debug = False):
        ...
        while len(self.ips) > 0:
            try:
                ...
                if len(self.ips) == 1:
                    raise e
                if self.verbose:
                    logit.log('Connection timeout on %s ip %s, trying %s' % (self.host, self.ips[0], self.ips[1]))
                del self.ips[0]
                self.newpool()
            else:
                if resp.status == 400 and ignore_400:
                    if self.debug:
                        logit.log("ignoring 400 status")
                    return resp
