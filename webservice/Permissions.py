...
class Permissions:
    ...
    def check_experiment_role(self, dbhandle, username, experiment, desired_role):
        ...
        if not key in self.excache:
            rows = (dbhandle.query(ExperimentsExperimenters.role).join(Experimenter, Experimenter.experimenter_id == ExperimentsExperimenters.experimenter_id).filter(
                Experimenter.username == username, ExperimentsExperimenters.experiment == experiment)).all()
            if rows:
    ...
    def get_exp_owner_role(self, dbhandle, t, item_id=None, name=None, experiment=None, campaign_id=None):
        ...
        if t == "Submission":
            ...
            q = (dbhandle.query(CampaignStage.experiment, CampaignStage.creator, CampaignStage.creator_role).join(Submission, Submission.campaign_stage_id == CampaignStage.campaign_stage_id))
            if item_id:
